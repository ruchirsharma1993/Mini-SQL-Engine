...
public class Test1 
{
	public static void main(String args[])
	{
		...
		"WHERE e.salary > 1000 AND e=10\n" +
		"ORDER BY\n" +
		" e.first_name,\n" +
		" e.last_name;";*/
		
		sqlparser.sqltext = "select a,b from table_name,tabl2 where table1.A>table1.B OR B>C";
			...
			for(int i=0;i<pStmt.joins.size();i++)
			{
				...
				switch (join.getKind())
				{
					...
					case TBaseType.join_source_fake:
							System.out.printf("\ntable:fake \n\t%s, alias:%s\n",join.getTable().toString(),(join.getTable().getAliasClause() !=null)?join.getTable()
					...
					case TBaseType.join_source_table:
							...
							for(int j=0;j<join.getJoinItems().size();j++)
							{
								...
								System.out.printf("table: %s, alias:%s\n",joinItem.getTable().toString(),(joinItem.getTable().getAliasClause() !=null)?joinItem.getTable().getAliasClause
								().toString():"");
